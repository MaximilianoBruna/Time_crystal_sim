...
# Discard the first 20 seconds of transient chaos while the system winds up
transient_cutoff = 20
...
# Plot 1: Time Domain (The "M-Shape" auto-oscillations)
ax1.plot(t_steady, Sz_steady, color='#1f77b4', lw=1.5)
ax1.set_title("Simulated M-Shape Auto-Oscillations (Fig. 2c)", fontsize=14)
ax1.set_xlabel("Time (s)", fontsize=12)
ax1.set_ylabel(r"Electron Spin Polarization $S_z$", fontsize=12)
#ax1.set_xlim(30, 45) # Zoom in to see the specific wave shapes
ax1.grid(True, linestyle='--', alpha=0.7)

# Plot 2: Phase Space Portrait (The Limit Cycle)
tau = 25 # Delay step
ax2.plot(Sz_steady[:-tau], Sz_steady[tau:], color='black', lw=0.8)
ax2.set_title("Phase Portrait (Limit Cycle)", fontsize=14)
ax2.set_xlabel(r"$S_z(t)$", fontsize=12)
ax2.set_ylabel(r"$S_z(t - \tau)$", fontsize=12)
ax2.grid(True, linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig('Simulating Radau.pdf', format='pdf')
plt.show()